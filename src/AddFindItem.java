...
@WebServlet("/AddFindItemServlet")
public class AddFindItem extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (item.getFieldName().equals("findimg")) {
					File file = new File(dir, item.getName());
					item.write(file);
					findimg = item.getName();
				...
				} else if (item.getFieldName().equals("findplace")) {
					findplace = new String(item.getString().getBytes("8859_1"), "UTF-8");
				} else if (item.getFieldName().equals("storage")) {
					storage = new String(item.getString().getBytes("8859_1"), "UTF-8");
				} else if (item.getFieldName().equals("lostname")) {
					lostname = new String(item.getString().getBytes("8859_1"), "UTF-8");
				} else if (item.getFieldName().equals("finddate")) {
					finddate = new String(item.getString().getBytes("8859_1"), "UTF-8");
				} else if (item.getFieldName().equals("findname")) {
					findname = new String(item.getString().getBytes("8859_1"), "UTF-8");
		...
		try {
			...
			st.executeUpdate("insert into finditem(itemname,findplace,storage,lostname,finddate,findimg,findname,phone,id) value ('" + itemname + "','" + findplace + "','" + storage + "','"
					+ lostname + "','" + finddate + "','" + findimg + "','" + findname + "','" + phone + "','" + id +"')");
